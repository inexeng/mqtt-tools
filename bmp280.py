...
if (device.readS8(BMP280_REGISTER_CHIPID) == 0x58): # check sensor id 0x58=BMP280
    ...
    while True: # loop
        ...
        print("temperature:{:.2f}".format(temp)+" C  pressure:{:.2f}".format(press/100)+" mBar   altitude:{:.2f}".format(altitude)+" m")
        p1 = float(press)
        t1 = float(temp)
        p2 = p1/100
        t2 = t1-9
        ...
        print p3
        print t3
        pickle.dump(t3, open( "tloft.p", "wb" ))
        pickle.dump(p3, open( "pres.p", "wb" ))
        time.sleep(10)
